...
<style>
    ...
    .css-1d391kg, .css-1v0mbdj, .css-1lcbmhc {background-color: #0E1117 !important; color: #E0E0E0 !important;}
...
st.markdown("<div style='text-align: center; color: #BBBBBB;'>AI-Powered Weekly Forecast | Auto-Updated Every Sunday</div>", unsafe_allow_html=True)
...
if page == "Dashboard":
    ...
    if total < 5: rec, color, label = "MINIMAL", "#4CAF50", "MINIMAL"
    elif total < 20: rec, color, label = "LIGHT", "#FFC107", "LIGHT"
    elif total < 40: rec, color, label = "MODERATE", "#FF9800", "MODERATE"
    else: rec, color, label = "HEAVY", "#F44336", "HEAVY"
    ...
    st.markdown(f"<div style='background-color: {color}22; padding: 1.5rem; border-radius: 0.5rem; border-left: 4px solid {color};'><h3 style='margin: 0; color: {color};'>{label} Irrigation This Week</h3><p><b>Apply:</b> {total:.0f} mm ({split_text})</p></div>", unsafe_allow_html=True)
    if not irrigation_days.empty:
        ...
        st.markdown(f"<div style='background-color: #1E2130; padding: 1rem; border-radius: 0.5rem; border-left: 4px solid #64B5F6; margin-top: 1rem;'><h4 style='margin: 0; color: #64B5F6;'>Recommended Irrigation Days</h4><p style='margin: 0.5rem 0; color: #E0E0E0;'>{schedule}</p><p style='font-size: 0.9rem; color: #AAAAAA;'><i>Only irrigate on days with >0.5 mm need. Rain covers the rest.</i></p></div>", unsafe_allow_html=True)
    else:
    ...
    fig = go.Figure()
    fig.add_trace(go.Bar(x=forecast_7day['date'].dt.strftime('%b %d'), y=forecast_7day['irrigation_mm'], marker_color='#66BB6A', text=forecast_7day['irrigation_mm'].round(1), textposition='outside'))
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=monthly.index, y=monthly.values, mode='lines+markers', line=dict(color='#64B5F6', width=3)))
    ...
    with col2:
        fig = go.Figure(go.Scattermapbox(lat=[-1.5789], lon=[30.5089], mode='markers', marker=go.scattermapbox.Marker(size=20, color='red'), text=['Gabiro Irrigation Scheme'], hoverinfo='text'))
...
# ============================================================================
# FIXED FOOTER — ALWAYS VISIBLE & STYLED
# ============================================================================
